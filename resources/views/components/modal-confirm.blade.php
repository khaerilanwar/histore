<div id="{{ $idModal }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
            ...
            <ion-icon name="alert" size="large" class="text-gray-500 w-full"></ion-icon>

            <p class="mb-4 text-gray-500 "> {{ $slot }} </p>
            <div class="flex justify-center items-center space-x-4">
                <button data-modal-toggle="{{ $idModal }}" type="button"
                ...
                @php
                    $namaModal = preg_replace('/\d/', '', $idModal);
                    preg_match_all('/\d+/', $idModal, $matches);
                    $nikUser = $matches[0][0];
                @endphp

                @if ($namaModal == 'resignModal')
                    <form action="/admin/staff/resign/{{ $nikUser }}" method="post">
                        @csrf
                        @method('patch')
                        <button type="submit"
